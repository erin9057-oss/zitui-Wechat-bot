...
export const DEFAULT_BASE_DIR = path.resolve(__dirname, '..');
...
// 修改 runtime-config.js 中的这两个函数
export function 获取角色名() {
    const config = 读取JSON文件(获取主配置路径());
    return config.profile?.char_name || null; // 🌟 必须返回 null，方便 bot.js 判断
}

export function 获取用户名() {
    const config = 读取JSON文件(获取主配置路径());
    return config.profile?.user_name || null;
}
